...
| Patient detail tab switching | ✅ The tabs are now real routes, not `?tab=` query params — two earlier fixes (a `key`-ed `<Suspense>`, then client-side `useSearchParams` switching) both worked around a query-param-only navigation never showing a loading state; moving to actual segments was the real fix, since Next.js only streams `loading.tsx` across genuine route navigations. `app/patients/[id]/(tabs)/layout.tsx` holds the shared chrome (`DashboardShell`, `PatientProfileHeader`, `PatientTabs`) and fetches `patient` once; it stays mounted across tab clicks, so switching tabs re-renders only the target segment. Each tab is its own route with a sibling `loading.tsx` and a server component `page.tsx` that fetches just what it needs: `(tabs)/page.tsx` (overview), `(tabs)/progress`, `(tabs)/casual`, `(tabs)/insights`, `(tabs)/memories`, `(tabs)/reminders`. `memories/add/page.tsx` is deliberately kept **outside** the `(tabs)` route group (same URL prefix, sibling folder) so it doesn't inherit the tabs chrome around what's meant to be a focused full-screen form. `PatientTabs` builds path-based links (`usePathname`) instead of query-string ones. |
