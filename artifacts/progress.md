...
| UI kit | `Screen` (one-row app bar, back arrow, `stickyHeader` — D-31), `Surface` (with `elevated`), `Section`, `SettingCard`/`SettingField`, `Text`, `ActionButton`, `ChoiceGroup`, `PreviewChoice`, `ColorChoice`, `StepSlider`, `Toggle`, `CodeInput`, `EmptyState`, `ProgressBar` (with a `durationMs` countdown mode), `Dialog` (scrolling, safe-area inset, with a `details` slot), `Confetti`, `SettingsGroup`/`Row`/`Link`/`Accordion`, `TextField`, `FieldTrigger`, `SelectField` (the platform's own dropdown — D-43), `TimeField` / `DateField` over `SchedulePicker` (Material 3's date and time dialogs on Android, dropdowns elsewhere — D-43), `RepeatField` (seven day toggles — D-42), `NativeHost`, `AppIcons` |
...
| **Games** | ✅ list of games at `(tabs)/games.tsx` — now a tab of its own, in the slot People held — mapped from `GameCatalogue`, also reached from the fourth tile of the games grid on Today; two cards on it — `decisions.md` D-18, D-41 |
...
6. ~~**Sync client**~~ — done 2026-09-01, D-33 (up) and D-34 (down: caregiver reminders and
   ...
   screen that draws the circle — the People tab was removed for Games (D-41), so that
   screen needs a home first; and reminder definitions going **up**, so a reminder
