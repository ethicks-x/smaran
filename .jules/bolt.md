...
## 2024-11-21 - N+1 Queries in Attention Flags Generation
**Learning:** `get_attention_flags` sequentially executed 3 scalar queries (`func.max` and filtering recent sessions) for each patient within a loop to calculate attention metrics. Because SQLAlchemy `AsyncSession` does not allow concurrent queries (and looping runs sequentially anyway), checking a caregiver's 10 patients generated 30 sequential DB trips, creating a large latency spike.
**Action:** When aggregating metrics per patient for a feed/dashboard, push the calculation to the database layer for *all* patients at once by replacing loop-bound scalar queries with grouped queries (`group_by(patient_id)` or `.in_(target_ids)`) prior to the loop. Then map the results in memory.
