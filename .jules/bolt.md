...
## 2025-02-28 - N+1 Query in Patient Loop for Attention Flags
**Learning:** `get_attention_flags` iterates over all patients and executed three individual `.scalar()` or `.scalars()` queries sequentially inside the loop (`SessionEvent.ended_at`, `GameSession.started_at`, and recent baseline `SessionEvent` history). This resulted in an O(3N) query bottleneck that grows strictly linearly with the user's patient load.
**Action:** Consistently push aggregation and selection up to SQLAlchemy. Outside the loop, query events for the set of patients using `.in_()` paired with `.group_by(patient_id)`, parse results into Python dictionaries mapping `patient_id` -> `data`, and retrieve those mapped results in O(1) time within the loop.
