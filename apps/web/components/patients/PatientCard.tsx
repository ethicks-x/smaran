...
import { getSessionsForPatient, getPatientAccuracy, getMemorySubjects } from "@/lib/mock-data";
    ...
    >
      <div className="flex items-start justify-between">
        <div className="flex items-center gap-3.5">
          ...
          />
          <div>
            <p className="font-display text-base font-semibold text-ink-900">{patient.full_name}</p>
      ...
      <div className="mt-4 grid grid-cols-2 gap-3">
        <div className="rounded-xl bg-black/[0.03] px-3 py-2.5">
          <div className="flex items-center gap-1.5 text-ink-500">
            <Gamepad2 size={13} />
            <span className="text-[11px] font-medium uppercase tracking-wide">Sessions</span>
          </div>
          <p className="mt-1 text-sm font-semibold text-ink-900">{sessions.length} played</p>
        </div>
        <div className="rounded-xl bg-black/[0.03] px-3 py-2.5">
          <div className="flex items-center gap-1.5 text-ink-500">
            <Clock size={13} />
            <span className="text-[11px] font-medium uppercase tracking-wide">Accuracy</span>
          </div>
      ...
      <div className="mt-4 flex items-center justify-between border-t border-black/[0.06] pt-3.5">
        <span className="text-xs text-ink-500">{memoryCount} memory subjects</span>
