...
import { caregiver, getPatientsForCaregiver, questionEvents, getPatient } from "@/lib/mock-data";
...
export default function ActivityPage() {
  ...
  return (
    <DashboardShell>
      <div className="mb-6">
        <h1 className="font-display text-2xl font-bold text-ink-900 sm:text-3xl">Activity & Progress</h1>
        <p className="mt-1.5 text-sm text-ink-500">
          Every recognition question asked across everyone in your care.
        </p>
      </div>

      <div className="mb-6 flex flex-wrap items-center gap-3 rounded-2xl border border-black/[0.06] bg-surface p-3">
        ...
        <select
          ...
          className="rounded-xl border border-black/[0.08] bg-white px-3 py-2 text-xs font-medium text-ink-700 focus:outline-none"
        >
      ...
      <div className="space-y-8">
        {Object.entries(grouped).map(([day, events]) => (
          <div key={day}>
            <p className="mb-3 text-xs font-semibold uppercase tracking-wide text-ink-500">{day}</p>
            <div className="space-y-2.5">
              {events.map((e) => {
                ...
                return (
                  <div
                    key={e.id}
                    className="flex items-center justify-between gap-4 rounded-2xl border border-black/[0.06] bg-surface p-4 shadow-[0_2px_8px_rgba(44,31,88,0.06)]"
                  >
                    <div className="flex items-center gap-3.5">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-ink-500">
                          ...
                          {new Date(e.asked_at).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}
                        </p>
                      </div>
                    </div>
        ...
        {filtered.length === 0 && (
          <div className="rounded-2xl border border-dashed border-black/15 bg-surface py-16 text-center">
            <p className="font-display text-lg font-semibold text-ink-900">No activity found</p>
            <p className="mt-1 text-sm text-ink-500">Try a different patient filter.</p>
          </div>
        )}
      </div>
    </DashboardShell>
