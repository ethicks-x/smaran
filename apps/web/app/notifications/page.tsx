  ...
  useEffect(() => {
    const fetchNotifications = async (isBackground = false) => {
      try {
        ...
        setItems(
          data.map((n) => (readIds.includes(n.id) ? { ...n, read: true } : n)),
        );
      } catch (err) {
          ...
          return (
            <div
              ...
              className={cn(
                ...
                n.read
                  ? "border-black/[0.06] dark:border-white/[0.08] bg-surface"
                  : "border-indigo-200 dark:border-indigo-400/30 bg-indigo-50/40",
              )}
            >
              <span
                className={cn(
